...
int max77978_i2c_bulk(const char *caller, u8 type, struct i2c_client *i2c, u8 reg, int count, u8 *buf)
{
	...
	for (i = 0, offset = 0; i < loop; i++, offset += MAX77978_I2C_BULK_BLOCK_SIZE) {
		...
		sprintf(strdump, "%s.%s.%02Xh : ", caller, I2C_BULK_STR[type], addr);
		...
		case MAX77978_I2C_OPCODE_WRITE:
			...
			if (err) {
				...
				sprintf(strdump, "%s.%s.%02X.", caller, I2C_BULK_STR[type], addr);
	...
	if (type == MAX77978_I2C_OPCODE_WRITE) {
		sprintf(strdump, "%s.%s.%02Xh : ", caller, I2C_BULK_STR[type], OPCODE_WRITE);
...
static int of_max77978_dt(struct device *dev, struct max77978_platform_data *pdata)
{
	...
	pdata->irq_gpiod = gpiod;
	pdata->wakeup = device_property_read_bool(dev, "wakeup");

	dev_info(dev, "%s: irq-gpiod: %s\n", __func__,
		 gpiod ? "configured" : "not configured");
	return 0;
}
...
static int max77978_i2c_probe(struct i2c_client *i2c)
{
	...
	if (pdata) {
		...
		max77978->irq_gpiod = pdata->irq_gpiod;
	...
	init_completion(&max77978->wait_apcmd_resp_completion);
	max77978->wait_apcmd_resp_work_queue = create_singlethread_workqueue("wait_apcmd_resp_work");
