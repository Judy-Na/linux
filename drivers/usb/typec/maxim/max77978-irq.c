...
static irqreturn_t max77978_irq_thread(int irq, void *data)
{
	...
	if (max77978->suspended) {
		pr_err("%s:%s skip.max77978 suspended. irq gpio post-state(%d)\n",
			MFD_DEV_NAME, __func__, gpiod_get_value(max77978->irq_gpiod));
	...
	pr_info("%s: irq gpio pre-state(%d)\n", __func__, gpiod_get_value(max77978->irq_gpiod));
...
int max77978_irq_init(struct max77978_dev *max77978)
{
	...
	max77978->irq = gpiod_to_irq(max77978->irq_gpiod);
