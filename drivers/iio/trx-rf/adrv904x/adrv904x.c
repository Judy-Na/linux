...
#define ADRV904X_INIT_CALS_TIMEOUT_MS	60000
...
int __adrv904x_dev_err(struct adrv904x_rf_phy *phy, const char *function,
		       const int line)
{
	int ret = 0;
				  ...
				  const char *buf, size_t len)
{
	...
	case ADRV904X_INIT_CAL:
		...
		if (val) {
			...
			ret = adrv904x_api_call(phy, adi_adrv904x_InitCalsWait,
						ADRV904X_INIT_CALS_TIMEOUT_MS);
		}
		break;
				     ...
				     const char *buf, size_t len)
{
	...
	case RX_QEC:
		...
		ret = adrv904x_api_call(phy, adi_adrv904x_TrackingCalsEnableSet,
				       calMask, mask,
	...
	case RX_DIG_DC:
		if (chan->channel > CHAN_RX8)
			return len;
		mask = ADI_ADRV904X_RX0 << chan->channel;
		ret = adrv904x_api_call(phy, adi_adrv904x_DigDcOffsetEnableSet,
				       mask, enable ? 0xFF : 0x00);
		if (ret)
			return ret;
		return len;
	case RX_ADC:
				    ...
				    const struct iio_chan_spec *chan, char *buf)
{
	...
	case RX_QEC:
		if (chan->channel > CHAN_RX8)
			return -EINVAL;
		ret = adrv904x_api_call(phy, adi_adrv904x_TrackingCalsEnableGet,
				       &enableMasks);
		if (ret)
			return ret;
	...
	case RX_DIG_DC:
		mask = ADI_ADRV904X_RX0 << chan->channel;
		ret = adrv904x_api_call(phy, adi_adrv904x_DigDcOffsetEnableGet,
				       mask, &isEnabled);
		if (ret)
			return ret;
		return sysfs_emit(buf, "%d\n", isEnabled);
				    ...
				    const struct iio_chan_spec *chan, char *buf)
{
	...
	ret = adrv904x_api_call(phy, adi_adrv904x_TrackingCalsEnableGet, enableMasks);
	if (ret)
		return ret;
	...
	switch (private) {
	case TX_QEC:
		return sysfs_emit(buf, "%d\n", !!(chan_cals & ADI_ADRV904X_TC_TX_QEC_MASK));
	case TX_LOL:
		return sysfs_emit(buf, "%d\n", !!(chan_cals & ADI_ADRV904X_TC_TX_LOL_MASK));
	case TX_LB_ADC:
		return sysfs_emit(buf, "%d\n", !!(chan_cals & ADI_ADRV904X_TC_TX_LB_ADC_MASK));
	default:
		return -EINVAL;
	}
}
				 ...
				 int *val2, long m)
{
	...
	case IIO_CHAN_INFO_ENABLE:
		ret = adrv904x_api_call(phy, adi_adrv904x_RxTxEnableGet,
				       &orxchan, &rxchan, &txchan);
		if (ret)
			return ret;
	...
	case IIO_CHAN_INFO_HARDWAREGAIN:
		if (chan->output) {
			...
			ret = adrv904x_api_call(phy, adi_adrv904x_TxAttenGet,
			...
			*val2 = (txAtten.txAttenuation_mdB % 1000) * 1000;
			...
			if (chan->channel <= CHAN_RX8) {
				...
				ret = adrv904x_api_call(phy, adi_adrv904x_RxGainGet,
	...
	case IIO_CHAN_INFO_FREQUENCY: {
		...
		if (chan->type == IIO_ALTVOLTAGE) {
			...
			ret = adrv904x_api_call(phy, adi_adrv904x_LoFrequencyGet, &loConfig);
			if (ret)
				return ret;
			freq = loConfig.loFrequency_Hz;
		} else if (chan->output) {
			freq = phy->adi_adrv904x_device.initExtract.tx.txChannelCfg[chan->channel].rfBandwidth_kHz;
			freq *= 1000;
		} else {
			freq = phy->adi_adrv904x_device.initExtract.rx.rxChannelCfg[chan->channel].rfBandwidth_kHz;
			freq *= 1000;
		}
...
static int adrv904x_write_tx_gain(struct adrv904x_rf_phy *phy,
				  const struct iio_chan_spec *chan,
				  int val, int val2)
{
	adi_adrv904x_TxAtten_t txAtten;
	...
	txAtten.txAttenuation_mdB = code;

	return adrv904x_api_call(phy, adi_adrv904x_TxAttenSet, &txAtten, 1);
}

static int adrv904x_write_rx_gain(struct adrv904x_rf_phy *phy,
				  const struct iio_chan_spec *chan,
				  int val, int val2)
{
	adi_adrv904x_RxGain_t rxGain;
	...
	ret = adrv904x_gain_to_gainindex(phy, chan->channel, val, val2, &code);
	if (ret < 0)
		return ret;
	...
	return adrv904x_api_call(phy, adi_adrv904x_RxGainSet, &rxGain, 1);
}
				  ...
				  int val2, long mask)
{
	...
	case IIO_CHAN_INFO_ENABLE:
		...
		orxchan_msk = 0;
		if (chan->output) {
			txchan_msk = BIT(chan->channel);
			if (val)
				txchan_en |= BIT(chan->channel);
			else
				txchan_en &= ~BIT(chan->channel);
		} else {
			if (chan->channel <= CHAN_RX8) {
				rxchan_msk = BIT(chan->channel);
				if (val)
					rxchan_en |= BIT(chan->channel);
				else
					rxchan_en &= ~BIT(chan->channel);
			} else {
				orxchan_msk = BIT(chan->channel);
				if (val)
					orxchan_en |= BIT(chan->channel);
				else
					orxchan_en &= ~BIT(chan->channel);
			}
		}
		return adrv904x_api_call(phy, adi_adrv904x_RxTxEnableSet,
				       orxchan_msk, orxchan_en, rxchan_msk,
				       rxchan_en, txchan_msk, txchan_en);
	case IIO_CHAN_INFO_HARDWAREGAIN:
		if (chan->output)
			return adrv904x_write_tx_gain(phy, chan, val, val2);
		return adrv904x_write_rx_gain(phy, chan, val, val2);
	case IIO_CHAN_INFO_FREQUENCY: {
		adi_adrv904x_LoConfig_t loConfig = { 0 };

		loConfig.loFrequency_Hz = ((u64)val2 << 32) | (u32)val;
		loConfig.loName = ADI_ADRV904X_LO0 + chan->channel;
		return adrv904x_api_call(phy, adi_adrv904x_LoFrequencySet, &loConfig);
	}
	case IIO_CHAN_INFO_SAMP_FREQ:
		return -EINVAL;
	default:
...
static int adrv904x_phy_write_raw_get_fmt(struct iio_dev *indio_dev,
...
static const struct iio_info adrv904x_phy_info = {
	...
	.write_raw_get_fmt = &adrv904x_phy_write_raw_get_fmt,
				 ...
				 u32 source)
{
	...
	_parent_name[0] = adrv904x_clk_set_dev_name(phy, p_name[0], parent_name);
	_parent_name[1] = adrv904x_clk_set_dev_name(phy, p_name[1], parent_name2);
	...
	case RX_SAMPL_CLK:
		...
		break;
	case OBS_SAMPL_CLK: {
		const adi_adrv904x_FramerSettingsExtract_t *framer =
			&phy->kororDevice->initExtract.jesdSetting.framerSetting[1];

		phy->orx_iqRate_kHz = framer->iqRate_kHz;
		init.ops = &bb_clk_ops;
		clk_priv->rate = phy->orx_iqRate_kHz;
		break;
	}
	case TX_SAMPL_CLK:
		ret = adrv904x_api_call(phy, adrv904x_TxLinkSamplingRateFind,
				       ADI_ADRV904X_DEFRAMER_0, &rate);
		if (ret)
			return ret;
...
static void adrv904x_info(struct adrv904x_rf_phy *phy)
{
	...
	dev_info(&spi->dev, "\nADRV9040 Rev %d, API version: %u.%u.%u.%u successfully initialized%s",
					   ...
					   enum jesd204_state_op_reason reason)
{
	...
	dev_clk = clk_round_rate(phy->dev_clk, deviceClockScaled_kHz * 1000);

	if (dev_clk > 0 && ((dev_clk / 1000) == deviceClockScaled_kHz)) {
		ret = clk_set_rate(phy->dev_clk, (unsigned long)dev_clk);
		if (ret)
			return ret;
	} else {
		dev_err(dev, "Requesting device clock %u failed got %ld",
					...
					enum jesd204_state_op_reason reason)
{
	...
	struct device *dev = jesd204_dev_to_device(jdev);
	struct adrv904x_rf_phy *phy = priv->phy;
	adi_adrv904x_ErrAction_e recoveryAction;
	...
	if (recoveryAction != ADI_ADRV904X_ERR_ACT_NONE) {
		dev_err(dev, "ERROR adi_adrv904x_HwOpen failed in %s at line %d.\n", __func__,
			__LINE__);
	...
	if (recoveryAction != ADI_ADRV904X_ERR_ACT_NONE) {
		dev_err(dev, "ERROR adi_adrv904x_HwReset failed in %s at line %d.\n", __func__,
			__LINE__);
		return JESD204_STATE_CHANGE_ERROR;
	}
				      ...
				      struct jesd204_link *lnk)
{
	...
	{
		const adi_adrv904x_DeframerSettingsExtract_t *deframer;
		...
		if (recoveryAction != ADI_ADRV904X_ERR_ACT_NONE) {
			dev_err(dev, "ERROR adi_adrv904x_DeframerCfgGet failed in %s at line %d.\n",
				__func__, __LINE__);
			return JESD204_STATE_CHANGE_ERROR;
		}

		deframer = &phy->kororDevice->initExtract.jesdSetting.deframerSetting[source_id - 1];

		priv->link[lnk->link_id].source_id = source_id;
		phy->tx_iqRate_kHz = deframer->iqRate_kHz;
		rate = deframer->iqRate_kHz;
		lnk->num_lanes = hweight8(deframer->deserialLaneEnabled);
		lnk->num_converters = deframer->jesdM;
		lnk->bits_per_sample = deframer->jesdNp;
	...
	case FRAMER2_LINK_RX:
	{
		...
		if (recoveryAction != ADI_ADRV904X_ERR_ACT_NONE) {
			dev_err(dev, "ERROR adi_adrv904x_FramerCfgGet failed in %s at line %d.\n",
				__func__, __LINE__);
			return JESD204_STATE_CHANGE_ERROR;
		}

		const adi_adrv904x_FramerSettingsExtract_t *framer =
			&phy->kororDevice->initExtract.jesdSetting.framerSetting[source_id - 1];

		priv->link[lnk->link_id].source_id = source_id;
		priv->link[lnk->link_id].is_framer = true;
		rate = framer->iqRate_kHz;
		if (lnk->link_id == FRAMER0_LINK_RX)
			phy->rx_iqRate_kHz = rate;
		if (lnk->link_id == FRAMER1_LINK_RX)
			phy->orx_iqRate_kHz = rate;
		lnk->num_lanes = hweight8(framer->serialLaneEnabled);
		lnk->num_converters = framer->jesdM;
		lnk->bits_per_sample = framer->jesdNp;
				       ...
				       enum jesd204_state_op_reason reason)
{
	...
	ret = adrv904x_api_call(phy, adi_adrv904x_MultichipSyncSet, ADI_ENABLE);
	if (ret)
		return ret;
					 ...
					 enum jesd204_state_op_reason reason)
{
	...
	for (i = 0; i < 255; i++) {
		ret = adrv904x_api_call(phy, adi_adrv904x_MultichipSyncStatusGet,
				       &mcsStatus);
		if (ret)
			return ret;
					 ...
					 enum jesd204_state_op_reason reason)
{
	...
	/* MCS end sequence*/
	ret = adrv904x_api_call(phy, adi_adrv904x_MultichipSyncSet, ADI_DISABLE);
	if (ret)
		return ret;
	...
	ret = adrv904x_api_call(phy, adi_adrv904x_SerializerReset);
	if (ret)
		return ret;
					...
					struct jesd204_link *lnk)
{
	...
	if (priv->link[lnk->link_id].is_framer) {
		if (phy->kororDevice->devStateInfo.linkSharingEnabled == 1) {
			ret = adrv904x_api_call(phy, adi_adrv904x_FramerSysrefCtrlSet,
					       ADI_ADRV904X_FRAMER_1, ADI_DISABLE);
			if (ret)
				return ret;

			ret = adrv904x_api_call(phy, adi_adrv904x_FramerLinkStateSet,
					       ADI_ADRV904X_FRAMER_1, ADI_DISABLE);
			if (ret)
				return ret;
		...
		ret = adrv904x_api_call(phy, adi_adrv904x_DeframerSysrefCtrlSet,
			       (uint8_t)ADI_ADRV904X_ALL_DEFRAMER, ADI_DISABLE);
		if (ret)
			return ret;

		ret = adrv904x_api_call(phy, adi_adrv904x_DeframerLinkStateSet,
			       (uint8_t)ADI_ADRV904X_ALL_DEFRAMER, ADI_DISABLE);
		if (ret)
			return ret;
	}
					...
					struct jesd204_link *lnk)
{
	...
	if (!priv->link[lnk->link_id].is_framer) { /* DEFRAMER */
		...
		ret = adrv904x_api_call(phy, adi_adrv904x_InitCalsRun, &serdesCal);
		if (ret)
			return ret;

		/* Wait up to 60 seconds for ARM */
		ret = adrv904x_api_call(phy, adi_adrv904x_InitCalsWait, 60000);
		if (ret) {
					 ...
					 struct jesd204_link *lnk)
{
	...
	if (priv->link[lnk->link_id].is_framer) {
		ret = adrv904x_api_call(phy, adi_adrv904x_FramerStatusGet,
		...
		ret  = adrv904x_api_call(phy, adi_adrv904x_DfrmLinkConditionGet,
			       priv->link[lnk->link_id].source_id, &deframerLinkCondition);
		if (ret)
			return ret;
...
static ADI_API adi_adrv904x_InitCalibrations_e drv_cals_TrackingCalConvert(const adi_adrv904x_TrackingCalibrationMask_e trackingCal)
{
	...
	case ADI_ADRV904X_TC_TX_LOL_MASK:
		return ADI_ADRV904X_IC_TXLOL;
	case ADI_ADRV904X_TC_TX_QEC_MASK:
		return ADI_ADRV904X_IC_TXQEC;
	case ADI_ADRV904X_TC_TX_SERDES_MASK:
		return ADI_ADRV904X_IC_SERDES;
	case ADI_ADRV904X_TC_RX_ADC_MASK:
		return ADI_ADRV904X_IC_ADC_RX;
	case ADI_ADRV904X_TC_TX_LB_ADC_MASK:
		return ADI_ADRV904X_IC_ADC_TXLB;
	case ADI_ADRV904X_TC_ORX_ADC_MASK:
		return ADI_ADRV904X_IC_ADC_ORX;
	case ADI_ADRV904X_TC_RX_QEC_MASK:
		fallthrough;
	default:
		return (adi_adrv904x_InitCalibrations_e)0;
	}
}
					       ...
					       enum jesd204_state_op_reason reason)
{
	...
	u32 ALL_CHANNELS_MASK = GENMASK(7, 0);
	u32 NUM_TRACKING_CALS = 7;
	u32 init_chans = 0;
	u8 i, j;
	int ret;

	u32 trackingCalMask = ADI_ADRV904X_TC_RX_ADC_MASK     |
	...
	ret = clk_set_rate(phy->clks[RX_SAMPL_CLK], phy->rx_iqRate_kHz * 1000);
	if (ret)
		return ret;
	ret = clk_set_rate(phy->clks[OBS_SAMPL_CLK], phy->orx_iqRate_kHz * 1000);
	if (ret)
		return ret;
	ret = clk_set_rate(phy->clks[TX_SAMPL_CLK], phy->tx_iqRate_kHz * 1000);
	if (ret)
		return ret;
	...
	dev_dbg(dev, "Initialized RF channels: 0x%x (TX: 0x%x, RX: 0x%x, ORX: 0x%x)\n",
		 init_chans, tx_mask, rx_mask, orx_mask);

	ret = adrv904x_api_call(phy, adi_adrv904x_RxTxEnableSet,
			       orx_mask, orx_mask, rx_mask, rx_mask, tx_mask, tx_mask);
	if (ret)
		return ret;

	ret = adrv904x_api_call(phy, adi_adrv904x_RxTxEnableSet,
			       orx_mask, 0x00, rx_mask, 0x00, tx_mask, 0x00);
	if (ret)
		return ret;
	...
	ret = adrv904x_api_call(phy, adi_adrv904x_TxAttenSet, txAttenuation, 1);
	if (ret)
		return ret;

	memset(&initCalStatus, 0, sizeof(adi_adrv904x_InitCalStatus_t));

	ret = adrv904x_api_call(phy, adi_adrv904x_InitCalsDetailedStatusGet, &initCalStatus);
	if (ret)
		return ret;

	for (i = 0; i < NUM_TRACKING_CALS; ++i) {
		trackingCal = (adi_adrv904x_TrackingCalibrationMask_e)(1 << i);

		if (((u32)trackingCal & trackingCalMask) == 0) {
		...
		for (j = 0; j < ADI_ADRV904X_MAX_CHANNELS; ++j) {
			if ((initCalStatus.calsSincePowerUp[j] & currentInitCalMask) == 0) {
...
static int adrv904x_probe(struct spi_device *spi)
{
	...
	const char *names[ADI_ADRV904X_RX_GAIN_TABLE_ARR_MAX];
	u32 masks[ADI_ADRV904X_RX_GAIN_TABLE_ARR_MAX];
	...
	strscpy(phy->trxBinaryInfoPtr.cpuProfile.filePath, name,
	...
	strscpy(phy->trxBinaryInfoPtr.cpu.filePath, name,
		sizeof(phy->trxBinaryInfoPtr.cpu.filePath));

	if (device_property_read_string(&spi->dev, "adi,arm-dfe-firmware-name", &name))
		return dev_err_probe(&spi->dev, -EINVAL,
				     "error missing property: adi,arm-dfe-firmware-name\n");
	strscpy(phy->trxBinaryInfoPtr.dfeCpu.filePath, name,
		sizeof(phy->trxBinaryInfoPtr.dfeCpu.filePath));

	if (device_property_read_string(&spi->dev, "adi,stream-firmware-name", &name))
		return dev_err_probe(&spi->dev, -EINVAL,
				     "error missing property: adi,stream-firmware-name\n");
	strscpy(phy->trxBinaryInfoPtr.stream.filePath, name,
	...
	if (n > 0) {
		...
		if (ret)
			return dev_err_probe(&spi->dev, ret,
					     "failed to read adi,rx-gaintable-channel-masks\n");
	...
	if (IS_ERR(phy->linux_hal.reset_gpio))
		return dev_err_probe(&spi->dev, PTR_ERR(phy->linux_hal.reset_gpio),
	...
	phy->kororDevice->common.deviceInfo.id = 0;
	phy->kororDevice->common.deviceInfo.name = "ADRV9040";
	phy->kororDevice->common.deviceInfo.type = 0x00;

	ret = adrv904x_api_call(phy, adi_adrv904x_HwOpen, &phy->spiSettings);
	if (ret)
		return ret;

	ret = adrv904x_api_call(phy, adi_adrv904x_SpiVerify);
	if (ret)
		return ret;

	ret = adrv904x_api_call(phy, adi_adrv904x_ApiVersionGet, &apiVersion);
	if (ret)
		return ret;

	dev_info(&spi->dev,
		 "ADRV9040 Rev %d, API version: %u.%u.%u.%u found",
		 phy->kororDevice->devStateInfo.deviceSiRev,
		 apiVersion.majorVer, apiVersion.minorVer,
		 apiVersion.maintenanceVer, apiVersion.buildVer);

	if (apiVersion.majorVer > 1) {
		...
		case ADI_ADRV904X_EXTRACT_INIT_DATA_LEGACY_PROFILE_BIN:
			dev_dbg(&spi->dev, "Using the Default Init and PostMcsInit Structures\n");
			break;

		case ADI_ADRV904X_EXTRACT_INIT_DATA_POPULATED:
			dev_dbg(&spi->dev, "Using the Profile Init and PostMcsInit Structures\n");
			break;
	...
	phy->clk_data = devm_kzalloc(&spi->dev,
				     struct_size(phy->clk_data, hws, NUM_ADRV904X_CLKS),
				     GFP_KERNEL);
	if (!phy->clk_data)
		return -ENOMEM;
	phy->clk_data->num = NUM_ADRV904X_CLKS;

	ret = adrv904x_clk_register(phy, "-rx_sampl_clk", __clk_get_name(phy->dev_clk), NULL,
				    CLK_GET_RATE_NOCACHE | CLK_IGNORE_UNUSED,
				    RX_SAMPL_CLK);
	if (ret)
		return ret;

	ret = adrv904x_clk_register(phy, "-obs_sampl_clk", __clk_get_name(phy->dev_clk), NULL,
				    CLK_GET_RATE_NOCACHE | CLK_IGNORE_UNUSED,
				    OBS_SAMPL_CLK);
	if (ret)
		return ret;

	ret = adrv904x_clk_register(phy, "-tx_sampl_clk", __clk_get_name(phy->dev_clk), NULL,
				    CLK_GET_RATE_NOCACHE | CLK_IGNORE_UNUSED,
				    TX_SAMPL_CLK);
	if (ret)
		return ret;

	ret = devm_of_clk_add_hw_provider(&spi->dev, of_clk_hw_onecell_get,
					  phy->clk_data);
	if (ret)
		return ret;
...
static struct spi_driver adrv904x_driver = {
	.driver = {
		.name = "adrv9040",
		.of_match_table = adrv904x_of_match,
	},
