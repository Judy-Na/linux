			    ...
			    int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_OFFSET:
		if (chan->type == IIO_TEMP) {
			 ...
			 */
			*val = (int)div_s64(-273150LL * 3140000LL, ret);
			return IIO_VAL_INT;
		}
		return -EINVAL;
				 ...
				 int val, int val2, long mask)
{
	...
	case IIO_CHAN_INFO_SAMP_FREQ:
		...
		/*
		 * Changing SPD changes the device configuration for the same
		 * channel address, so drop the "already programmed" fast path:
		 * the next read must reprogram the second command byte with the
		 * new SPD bit.  LTC2497_CONFIG_DEFAULT never equals a real
		 * channel/temperature address, so it is a safe re-arm sentinel
		 * (as used at probe).
		 *
