...
The power mode is selected automatically when the sampling frequency is set.
The driver picks the lowest-power mode that supports the requested frequency.
...
* 0 µV - (AVDD1 - AVSS) / 2 (mid-supply, hardware default)
* 1,650,000 µV - 1.65V
* 2,500,000 µV - 2.5V
* 2,140,000 µV - 2.14V
...
+=======================================+==============================================================+
| ``in_voltage<N>_sampling_frequency``  | Requested sampling frequency in Hz for channel N. Enabled    |
|                                       | channels are grouped into up to two profiles at capture time.|
+---------------------------------------+--------------------------------------------------------------+
...
| Attribute                             | Description                                                  |
+=======================================+==============================================================+
| ``in_voltage<N>_filter_type``         | Requested filter type for channel N: "wideband" or "sinc5".  |
|                                       | It is grouped with sampling frequency at capture time.       |
+---------------------------------------+--------------------------------------------------------------+
...
+---------------------------------------+--------------------------------------------------------------+
| ``in_voltage<N>_convdelay``           | Per-channel conversion delay. The driver exposes the phase/  |
|                                       | modulator-delay register as a physical time (picoseconds).  |
|                                       | Resolution and valid range depend on the decimation ratio    |
|                                       | in use (see datasheet Table 30).                             |
+---------------------------------------+--------------------------------------------------------------+
